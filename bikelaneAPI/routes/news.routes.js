...
router.get( '/', async ( req, res ) => {
    ...
    try {
        ...
        const news = await News.find().populate('category').sort( [ [ 'newsdate', 1 ] ] );
...
router.post( '/admin', upload.single( "image" ), async ( req, res ) => {
    ...
    try {
        ...
        return res.status( 201 ).json( { message: routeTitle + ": Ny er oprettet", oprettet: news } );
...
router.put( '/admin/:id', upload.single( "image" ), async ( req, res ) => {
    ...
    try {
        ...
        if ( req.file ) {
            ...
            let e = await News.findById(req.params.id)
        ...
        return res.status( 201 ).json( { message: routeTitle + " er rettet", rettet: news } )
